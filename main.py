...
def draw_parent_frame(window, btn_handlers):
    global parent_frame
    global e1
    global e2
    ...
    btn_1 = ttk.Button(parent_frame, text="Search by Department", command=btn_handlers["search"]).grid(row=0, column=1, padx=15, pady=15)
    btn_2 = ttk.Button(parent_frame, text="Delete Employee", command=btn_handlers["delete"]).grid(row=0, column=4, padx=15, pady=15)
    btn_home = ttk.Button(parent_frame, text="HOME", command=btn_handlers["home"]).grid(row=0, column=2, padx=15, pady=15)
    btn_update = ttk.Button(parent_frame, text="Update", command=btn_handlers["update"]).grid(row=1, column=0, padx=15, pady=15)

    return parent_frame


def draw_child_frame(employees):
    global child_frame
    global parent_frame
...
def btn_search():
    ...
    if len(result) != 0:
        draw_child_frame(result)
    else:
        draw_child_frame([])
        ttk.Label(child_frame, text="No Result Found", borderwidth=2, relief="solid", width=15).grid(column=0, row=0, padx=2) 
...
def btn_delete():
    ...
    employees = read_csv(FILE_PATH)
    draw_child_frame(employees[0: 10])
...
def btn_update():
    ...
    def btn_submit():
        ...
        employees = read_csv(FILE_PATH)
        draw_child_frame(employees[1: 10])
...
def main():
    ...
    draw_parent_frame(window, handlers)
    draw_child_frame(employees[0: 10])
